...
def install_apitrace(source_dir, prefix):
    name = 'apitrace'
    url = f'git@github.com:apitrace/{name}.git'
    version = ''
    ...
    os.chdir(str(dst))
    cmd = f'git submodule update --init --depth 1 --recursive'
    run_shell(cmd)

    configure = f'cmake -S. -Bbuild -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_INSTALL_PREFIX={prefix}'
    cmd = f'cmake {configure}'
    run_shell(cmd)

    cmd = f'make -j 8 -C build'
    run_shell(cmd)
...
def main() -> int:
    ...
    install_apitrace(source_dir, prefix)
