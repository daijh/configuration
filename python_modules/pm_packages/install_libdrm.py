...
def install_libaom(source_dir, prefix):
    name = 'drm'
    url = f'https://gitlab.freedesktop.org/mesa/drm.git'
    version = ''
    configure = f''
    ...
    cmd = f'meson configure {builddir} -Dprefix={prefix} -Dinstall-test-programs=true'
    run_shell(cmd)

    cmd = f'ninja -C {builddir} install'
    run_shell(cmd)
...
def main() -> int:
    ...
    install_libaom(source_dir, prefix)
